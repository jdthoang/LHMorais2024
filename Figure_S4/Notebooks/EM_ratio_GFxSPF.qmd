...
folder_path <- file.path("..", "..", "Data", "GFxSPF_ASO", "EM")
...
interaction <- paste(effect_1, effect_2, sep = "*")
rand_eff <- paste("(1", 'Subject)', sep="|")
rhs <- paste(sum, interaction, rand_eff, sep = " + ")
...
write.csv(lv_mod_params, 
          file = file.path("..",
                           "Output", 
                           glue("mitochonrial_ratio-{Sys.Date()}-params.csv")),
          quote = FALSE)
...
write.csv(lv_mod_contrasts, 
          file = file.path("..",
                           "Output", 
                           glue("mitochonrial_ratio-{Sys.Date()}.csv")),
          quote = FALSE)
```
