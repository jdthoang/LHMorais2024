...
ggplot(data, aes(x = eval(as.name(groups)), y = eval(as.name(variable)))) +
  ...
  geom_point(
    ...
    alpha = 0.9) +
  labs(x = NULL, y = "OCR (pmol/sec)", title ="Seahorse_Auronafin_CI_Uncoupled") +
...
kruskwall_output <- file.path("..", 
                              "Output", 
                              glue("KW_CI_Uncoupled-{Sys.Date()}-kruskal.txt"))
...
if (kruskal_result$p.value < 0.05) {
  ...
  file_name <- file.path("..", 
                         "Output", 
                         glue("KW_CI_Uncoupled-{Sys.Date()}-pairwise.txt"))
