---
title: "Linear Models mt markers"
format: html
editor: visual
...
excel_file <- file.path("..", "..", "Data", "GFxSPF_ASO", "WB_Striatum_mitomarkers_GFSPFASO_032524.xlsx")
...
variable = 'Grp75_betaactin_relativetoWT'
effect_1 = 'Microbiome'
effect_2 = 'Genotype'
groups = 'Group'

data$Group <- paste(data[[effect_2]], data[[effect_1]], sep='-')
...
ggplot(data, aes(x = eval(as.name(groups)), y = eval(as.name(variable)))) +
  ...
  labs(x = NULL, y = "Grp75_betaactin_relativetoWT") +
...
ggplot(data, 
  ...
  labs(title = "ECDF of signal of Grp75 β-actin relative to WT",
       y = NULL,
       x = "Grp75 β-actin relative to WT",
       color = groups) +
  theme_light()
```
...
write.csv(lv_mod1_params, 
          file = file.path("..", 
                           "Output", 
                           glue("lmGrp75_betaactin_relativetoWT-{Sys.Date()}-params.csv")),
          quote = FALSE)
...
write.csv(lv_mod1_contrasts, 
          file = file.path("..", 
                           "Output", 
                           glue("lmGrp75_betaactin_relativetoWT-{Sys.Date()}.csv")),
          quote = FALSE)
```
