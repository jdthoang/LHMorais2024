...
ggplot(data, aes(x = eval(as.name(groups)), y = eval(as.name(variable)))) +
  ...
  labs(x = NULL, y = "Mitochondrial complex IV activity") +
...
ggplot(data, 
  ...
  labs(title = "ECDF of mt complex IV activity",
       y = NULL,
       x = "Complex IV total activity",
       color = groups) +
  theme_light()
```
...
write.csv(lv_mod1_params, 
          file = file.path("..",  
                           "Output", 
                           glue("lmCytC-{Sys.Date()}-params.csv")),
          quote = FALSE)
