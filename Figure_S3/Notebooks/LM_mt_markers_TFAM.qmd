...
variable = 'TFAM_betaactin_relativetoWT'
effect_1 = 'Microbiome'
effect_2 = 'Genotype'
groups = 'Group'
...
ggplot(data, aes(x = eval(as.name(groups)), y = eval(as.name(variable)))) +
  ...
  labs(x = NULL, y = "TFAM_betaactin_relativetoWT") +
...
ggplot(data, 
  ...
  labs(title = "ECDF of signal of TFAM β-actin relative to WT",
       y = NULL,
       x = "TFAM β-actin relative to WT",
       color = groups) +
  theme_light()
```
...
write.csv(lv_mod1_params, 
          file = file.path("..",  
                           "Output", 
                           glue("lmTFAM_betaactin_relativetoWT-{Sys.Date()}-params.csv")),
          quote = FALSE)
...
write.csv(lv_mod1_contrasts, 
          file = file.path("..", 
                           "Output", 
                           glue("lmTFAM_betaactin_relativetoWT-{Sys.Date()}.csv")),
          quote = FALSE)
```
