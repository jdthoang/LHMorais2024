...
ggplot(data, aes(x = eval(as.name(groups)), y = eval(as.name(variable)))) +
  ...
  labs(x = NULL, y = "Mitochondrial complex I activity") +
...
ggplot(data, 
  ...
  labs(title = "ECDF of mt complex I activity",
       y = NULL,
       x = "Complex I total activity",
       color = groups) +
  theme_light()
```
...
write.csv(lv_mod1_params, 
          file = file.path("..", 
                           "Output", 
                           glue("lmNADH-{Sys.Date()}-params.csv")),
          quote = FALSE)
