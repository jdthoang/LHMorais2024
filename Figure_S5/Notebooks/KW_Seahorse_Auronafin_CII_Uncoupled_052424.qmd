...
p <-ggplot(data, aes(x = Group, y = CII_Uncoupled)) +
  geom_boxplot() +
  geom_point(
         aes(color = Group), 
...
p <- p + ggtitle("Seahorse_Auronafin_CII_Uncoupled")
print(p)
```

```{r}
# Kruskal-Wallis test
kruskal_result <- kruskal.test(CII_Uncoupled ~ Group, data = data)

# Write Kruskall-Wallis test results into a .txt file
kruskwall_output <- file.path("..", "..", "..",  "Output", "Seahorse_Auranofin", "CII", glue("KW_CII_Uncoupled-{Sys.Date()}-kruskal.txt"))
...
if (kruskal_result$p.value < 0.05) {
  pairwise_result <- dunn.test(data$CII_Uncoupled, data$Group, method = "BH")
  ...
  file_name <- file.path("..", "..", "..",  "Output", "Seahorse_Auranofin", "CII", glue("KW_CII_Uncoupled-{Sys.Date()}-pairwise.txt"))
