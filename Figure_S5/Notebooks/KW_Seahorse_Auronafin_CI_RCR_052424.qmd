...
p <-ggplot(data, aes(x = Group, y = CI_RCR)) +
  geom_boxplot() +
  geom_point(
         aes(color = Group), 
...
p <- p + ggtitle("Seahorse_Auronafin_CI_RCR")
print(p)
```

```{r}
# Kruskal-Wallis test
kruskal_result <- kruskal.test(CI_RCR ~ Group, data = data)

# Write Kruskall-Wallis test results into a .txt file
kruskwall_output <- file.path("..", "..", "..",  "Output", "Seahorse_Auranofin", "CI", glue("KW_CI_RCR-{Sys.Date()}-kruskal.txt"))
...
if (kruskal_result$p.value < 0.05) {
  pairwise_result <- dunn.test(data$CI_RCR, data$Group, method = "BH")
  ...
  file_name <- file.path("..", "..", "..",  "Output", "Seahorse_Auranofin", "CI", glue("KW_CI_RCR-{Sys.Date()}-pairwise.txt"))
