...
ggplot(data, aes(x = eval(as.name(groups)), y = eval(as.name(variable)))) +
  ...
  labs(x = NULL, y = "OCR (pmol/sec)", title = "CI Striatum State 4") +
...
write.csv(lv_mod1_params, 
          file = file.path("..",
                           "Output", 
                           glue("lmTotalSeahorse_CI_Striatum_State4-{Sys.Date()}-params.csv")),
          quote = FALSE)
...
write.csv(lv_mod1_contrasts, 
          file = file.path("..", 
                           "Output", 
                           glue("lmTotalSeahorse_CI_Striatum_State4-{Sys.Date()}.csv")),
          quote = FALSE)
```
